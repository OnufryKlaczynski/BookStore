...
class CartTest(TestCase):
    ...
    def test_add_item_to_cart(self):
        self.assertEqual(self.cart_with_one_item.cart, {1:{'paper_book':{'price':Decimal("13.22", ), 'quantity':1 }}})
        self.assertEqual(self.cart_with_items.cart, {
            1:{
                'paper_book': {'price':Decimal("13.22", ), 'quantity':1 },
                'ebook': {'price':Decimal("13.22"), 'quantity':1},
                'audio_book': {'price':Decimal("13.22"), 'quantity':1},
                }
            })
